bl_info = {
    ...
    'version': (4, 6, 1),
    'blender': (2, 75, 0),
    'location': 'File > Export > babylon (.json)',
    'description': 'Export Babylon.js scenes for babylon (.json)',
...
def menu_func(self, context):
    self.layout.operator(Main.bl_idname, text = 'babylon [.json] ver ' + format_version())
...
COMPRESS_MATRIX_INDICES = True # this is True for .json exporter & False for TOB
...
class Main(bpy.types.Operator, bpy_extras.io_utils.ExportHelper):
    ...
    filename_ext = '.json'          # required to have one, although not really used
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    def to_scene_file(self):
        ...
        # Open file
        file_handler = io.open(self.filepathMinusExtension + '.json', 'w', encoding='utf8')
        file_handler.write('{')
        file_handler.write('"producer":{"name":"Blender","version":"' + bpy.app.version_string + '","exporter_version":"' + format_version() + '","file":"' + Main.nameSpace + '.json"},\n')

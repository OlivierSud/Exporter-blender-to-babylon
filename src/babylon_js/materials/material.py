...
class BJSMaterial:
    ...
    def __init__(self, mat, exporter):
        ...
        # Fix for AttributeError: 'BJSMaterial' object has no attribute 'node_tree'
        # isinstance(mat, BJSMaterial) can fail due to module reloading.
        # Use duck typing: Blender materials have 'node_tree', BJSMaterial does not.
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    def bakeChannel(self, bjs_type, bake_type, usePNG, node_trees, bpyMesh):
        ...
        # Ensure object is enabled for rendering during bake
        was_hide_render = bpyMesh.hide_render
        bpyMesh.hide_render = False

        try:
            bpy.ops.object.bake(type = bake_type, use_clear = True, margin = 5, use_selected_to_active = False)
        finally:
            # Restore original state
            bpyMesh.hide_render = was_hide_render
